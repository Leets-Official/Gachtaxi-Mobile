...
class _TermsAgreementModalState extends State<TermsAgreementModal> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        _buildCheckboxTile("약관 모두 동의", allChecked, (value) {
          _updateCheckbox(value, "all");
        }, isBold: true, isAllAgreement: true),
        const SizedBox(height: AppSpacing.spaceMedium),

        _buildCheckboxTile("이용 약관 동의 (필수)", termsChecked, (value) {
          _updateCheckbox(value, "terms");
        }),

        _buildCheckboxTile("개인정보 수집 및 이용 동의 (필수)", privacyChecked, (value) {
          _updateCheckbox(value, "privacy");
        }),
  ...
  Widget _buildCheckboxTile(String title, bool value, Function(bool?) onChanged, {bool isBold = false, bool isAllAgreement = false}) {
    return GestureDetector(
      onTap: () => onChanged(!value),
      child: isAllAgreement
          ? Container(
        padding: const EdgeInsets.symmetric(vertical: AppSpacing.SpaceExtraExtraMedium, horizontal: AppSpacing.spaceCommon),
